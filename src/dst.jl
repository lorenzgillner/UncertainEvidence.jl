...
function combine_dempster(X::Dict, Y::Dict)
    X = distribute(X); Y = distribute(Y)
    prod = Iterators.product(X, Y)
    es = Set(keys(X)) ∪ Set(keys(Y))
    k = 1 - sum(x[1].second * x[2].second for x in prod if isempty(x[1].first ∩ x[2].first))
    Dict([e => sum(x[1].second * x[2].second for x in prod if x[1].first ∩ x[2].first == e) / k for e in es])
end
